...
pub fn particle_to_tile(ecs: &mut World, tile_idx: i32, effect: &EffectSpawner) {
    ...
    } = effect.effect_type
    {
        ...
        let mut particle_builder = ecs.fetch_mut::<particle_system::ParticleBuilder>();
        particle_builder.request(tile_idx % map.width, tile_idx / map.width, fg, bg, glyph, lifespan);
    }
}
