...
pub fn clear_events() { EVENTS.lock().unwrap().clear(); }
...
pub fn get_event_count<T: ToString>(event: T) -> i32 {
    ...
    if let Some(e) = events.get(&event_name) { *e } else { 0 }
}

pub fn clone_events() -> HashMap<String, i32> { EVENTS.lock().unwrap().clone() }
