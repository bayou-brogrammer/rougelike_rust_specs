...
pub fn ranged_target_menu(gs: &mut State, ctx: &mut Rltk, range: i32) -> (ItemMenuResult, Option<Point>) {
    let player_entity = gs.ecs.fetch::<Entity>();
    let player_pos = gs.ecs.fetch::<Point>();
    let viewsheds = gs.ecs.read_storage::<Viewshed>();

    let (min_x, max_x, min_y, max_y) = map::camera::get_screen_bounds(&gs.ecs, ctx);
    ...
    draw_batch
        .submit(5000)
        .expect("Failed to submit draw batch ranged menu");
