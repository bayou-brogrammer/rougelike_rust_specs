...
impl<'a> System<'a> for EncumbranceSystem {
    ...
    fn run(&mut self, data: Self::SystemData) {
        let (mut equip_dirty, entities, items, backpacks, wielded, mut pools, attributes, player, mut gamelog) = data;
                ...
                if let Some(attr) = attributes.get(*entity) {
                    ...
                    if pool.total_weight as i32 > carry_capacity_lbs {
                        ...
                        if *entity == *player {
                            gamelog
                                .entries
                                .push("You are overburdened, and suffering an initiative penalty.".to_string());
                        }
                    }
                }
            }
